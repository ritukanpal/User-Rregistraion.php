<?php
// Notify vendor that a design was approved by the client
$config_locations = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    if (!hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')){
        ...
        if ($order_id === '' || $vendor_name === '' || !filter_var($vendor_email, FILTER_VALIDATE_EMAIL)){
            ...
            // Best-effort: update order record to mark design approved
            try {
                $db = getDB();
                ...
                $order = $stmt->fetch();
                if ($order) {
                    $u = $db->prepare("UPDATE orders SET design_status='approved', design_approved_at=NOW(), updated_at=NOW() WHERE id=?");
                    $u->execute([$order['id']]);
                } else {
                    $i = $db->prepare('INSERT INTO orders (order_id, vendor_name, vendor_email, design_status, created_at, design_approved_at) VALUES (?,?,?,?,?,NOW())');
                    $i->execute([$order_id, $vendor_name, $vendor_email, 'approved', date('Y-m-d H:i:s')]);
                }
            } catch (Exception $e) {
                // ignore DB errors
            }
            ...
            // Prepare email
            $tplPath = __DIR__ . '/Design Approved.html';
            if (file_exists($tplPath)){
                ...
                $html = "<p>Dear " . htmlspecialchars($vendor_name) . ",</p><p>The design submitted for Order " . htmlspecialchars($order_id) . " has been approved by the client.</p><p>Regards,<br>Team Dausto</p>";
            }

            $subject = 'Design Approved Successfully';
...
<head>
  ...
  <title>Notify Vendor: Design Approved — Dausto</title>
...
<body>
<div class="box">
  <h2>Notify Vendor: Design Approved</h2>
  <?php if ($error): ?>
    <div class="err"><?=htmlspecialchars($error)?></div>
  <?php endif; ?>

  <?php if ($success): ?>
    <div class="ok">Notification sent to <?=htmlspecialchars($vendor_email)?>.</div>
