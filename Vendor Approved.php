...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    if (!hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')){
        ...
        if (!filter_var($vendor_email, FILTER_VALIDATE_EMAIL) || $vendor_name === ''){
            $error = 'Please provide a valid vendor name and email.';
        } else {
            $db = getDB();
            // Try to mark existing user (portal=vendor) as active
            try {
                $stmt = $db->prepare("SELECT id, name, status FROM users WHERE email=? LIMIT 1");
                $stmt->execute([$vendor_email]);
                $user = $stmt->fetch();
                if ($user) {
                    // Update status to 'active' or 'approved'
                    $update = $db->prepare("UPDATE users SET status='active', updated_at=NOW() WHERE id=?");
                    $update->execute([$user['id']]);
                }
            } catch (Exception $e) {
                // ignore DB errors; continue to send email
            }

            // Load approval email template
            $tplPath = __DIR__ . '/Vendor Approval.html';
            if (file_exists($tplPath)){
                $html = file_get_contents($tplPath);
                $html = str_replace('{{Vendor Name}}', htmlspecialchars($vendor_name, ENT_QUOTES|ENT_HTML5), $html);
            } else {
                $html = "<p>Dear " . htmlspecialchars($vendor_name) . ",</p><p>Congratulations! Your vendor account has been approved. You can now log in and start managing your products and orders.</p><p>Regards,<br>Team Dausto</p>";
            }
...
<head>
  ...
  <title>Approve Vendor — Dausto</title>
...
<div class="box">
  ...
  <?php if ($success): ?>
    <div class="ok">Approval email sent to <?=htmlspecialchars($vendor_email)?>.</div>
