...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    if (!hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')){
        ...
        if ($order_id === '' || $client_name === '' || !filter_var($client_email, FILTER_VALIDATE_EMAIL)){
            ...
            // Attempt to set order status to 'accepted' in DB
            try {
                $db = getDB();
                ...
                $order = $stmt->fetch();
                if ($order) {
                    $u = $db->prepare("UPDATE orders SET status='accepted', vendor_accepted_at=NOW(), updated_at=NOW() WHERE id=?");
                    $u->execute([$order['id']]);
                } else {
                    // insert minimal record
                    $i = $db->prepare('INSERT INTO orders (order_id, customer_name, customer_email, status, created_at, vendor_accepted_at) VALUES (?,?,?,?,?,NOW())');
                    $i->execute([$order_id, $client_name, $client_email, 'accepted', date('Y-m-d H:i:s')]);
                }
            } catch (Exception $e) {
                // ignore DB errors
            }
            ...
            // Load template
            $tplPath = __DIR__ . '/Oder Accepted.html';
            if (file_exists($tplPath)){
                ...
                $html = "<p>Dear " . htmlspecialchars($client_name) . ",</p><p>Your order has been accepted and production will begin shortly.</p><p>Order ID: " . htmlspecialchars($order_id) . "</p><p>Regards,<br>Team Dausto</p>";
            }

            $subject = 'Order Accepted by Vendor';
...
<head>
  ...
  <title>Mark Order Accepted — Dausto</title>
...
<body>
<div class="box">
  <h2>Mark Order Accepted</h2>
  <?php if ($error): ?>
    <div class="err"><?=htmlspecialchars($error)?></div>
  <?php endif; ?>

  <?php if ($success): ?>
    <div class="ok">Acceptance notification sent to <?=htmlspecialchars($client_email)?>.</div>
